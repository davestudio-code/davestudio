...
local BaseURL = "https://gitlab.com/deividcomsono/Obsidian/-/raw/main/"
...
local Library = {
    ...
    Scheme = {
        ...
        MainColor = Color3.fromRGB(20, 22, 26),
        AccentColor = Color3.fromRGB(0, 255, 128),
        OutlineColor = Color3.fromRGB(36, 40, 48),
        FontColor = Color3.fromRGB(160, 165, 175),
...
function Library:GetTextBounds(Text: string, Font: any, Size: number, Width: number?): (number, number)
    local fontEnum = Enum.Font.RobotoMono
    ...
    local w = Width or (workspace.CurrentCamera.ViewportSize.X - 32)
    local bounds = TextService:GetTextSize(Text, Size, fontEnum, Vector2.new(w, 10000))
...
function Library:AddDraggableButton(Text: string, Func, ExcludeScaling: boolean?, ExcludeDragging: boolean?)
    ...
    Button.MouseEnter:Connect(function()
        TweenService:Create(Button, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(30, 32, 38), TextColor3 = Color3.fromRGB(0, 240, 140)}):Play()
        TweenService:Create(stroke, TweenInfo.new(0.2), {Color = Color3.fromRGB(60, 65, 75)}):Play()
    end)
    Button.MouseLeave:Connect(function()
        TweenService:Create(Button, TweenInfo.new(0.2), {BackgroundColor3 = Library.Scheme.MainColor, TextColor3 = Color3.fromRGB(0, 200, 100)}):Play()
        TweenService:Create(stroke, TweenInfo.new(0.2), {Color = Library.Scheme.OutlineColor}):Play()
    end)
    ...
    function Table:SetText(Text: string)
        local rawText = Text
        if Text:lower() == "davestudio" then
            rawText = '<font color="#FFFFFF">dave</font><font color="#00C864">studio</font>'
        end
        local textForBounds = string.gsub(Text, "<font.->", ""):gsub("</font>", "")
        local X, Y = Library:GetTextBounds(textForBounds, Font.fromEnum(Enum.Font.RobotoMono), 15)

        Button.Text = rawText
        Button.Size = UDim2.fromOffset(X + 24, Y + 18)
    end
    Table:SetText(Text)
...
do
    ...
    function Library:OpenModalSelector(Dropdown, Info)
        ...
        local TitleText = Dropdown.Text or "Items"
        if not string.find(string.lower(TitleText), "select") then
            TitleText = "Select " .. TitleText
        end

        local Emoji = "🏷️"
        local LowerTitle = string.lower(TitleText)
        if string.find(LowerTitle, "fruit") or string.find(LowerTitle, "plant") or string.find(LowerTitle, "seed") or string.find(LowerTitle, "watermelon") then
            Emoji = "🍉"
        end
        ...
        local InitialValues = {}
        if Info.Multi then
            for Val, Act in pairs(Dropdown.Value) do
                TempValues[Val] = Act
                InitialValues[Val] = Act
            end
        else
            TempValues = Dropdown.Value
            InitialValues = Dropdown.Value
        end

        local function UpdateDoneButtonText()
            local HasChanges = false
            if Info.Multi then
                for Val, Act in pairs(TempValues) do
                    if InitialValues[Val] ~= Act then
                        HasChanges = true
                        break
                    end
                end
                if not HasChanges then
                    for Val, Act in pairs(InitialValues) do
                        if TempValues[Val] ~= Act then
                            HasChanges = true
                            break
                        end
                    end
                ...
                HasChanges = (TempValues ~= InitialValues)
            end
            DoneButton.Text = HasChanges and "Done" or "Close"
        end

        local ItemButtons = {}
        local VisualUpdates = {}

        local function ApplyAndClose(FinalValue)
            Dropdown.Value = FinalValue
            Dropdown:Display()

            if Dropdown.Menu and Dropdown.Menu.Menu then
                for _, OtherButton in pairs(Dropdown.Menu.Menu:GetChildren()) do
                    if OtherButton:IsA("Frame") then
                        local Btn = OtherButton:FindFirstChildOfClass("TextButton")
                        if Btn then
                            local IsSelected
                            if Info.Multi then
                                IsSelected = Dropdown.Value[Btn.Text]
                            else
                                IsSelected = Dropdown.Value == Btn.Text
                            end
                            OtherButton.BackgroundTransparency = IsSelected and 0 or 1
        ...
        local function BuildModalItems()
            ...
            for _, Value in ipairs(Values) do
                ...
                if SearchText == "" or string.find(string.lower(FormattedValue), SearchText) then
                    ...
                    local function UpdateVisuals()
                        local IsSelected
                        if Info.Multi then
                            IsSelected = TempValues[Value]
                        else
                            IsSelected = (TempValues == Value)
                        end

                        if IsSelected then
                            ItemLabel.TextColor3 = Color3.fromRGB(200, 205, 215)
                            ItemLabel.TextTransparency = 0
                        else
                            ItemLabel.TextColor3 = Color3.fromRGB(150, 153, 160)
                            ItemLabel.TextTransparency = 0.25
                        end
                    end

                    VisualUpdates[Value] = UpdateVisuals

                    ItemBtn.MouseButton1Click:Connect(function()
                        if Info.Multi then
                            local clickedValue = Value
                            if TempValues[clickedValue] then
                                TempValues[clickedValue] = nil
                            else
                                TempValues[clickedValue] = true
                                if clickedValue == "None" or clickedValue == "All" then
                                    for otherVal, _ in pairs(TempValues) do
                                        if otherVal ~= clickedValue then
                                            TempValues[otherVal] = nil
                                        end
                                    end
                                else
                                    TempValues["None"] = nil
                            ...
                            for _, updateFunc in pairs(VisualUpdates) do
                                updateFunc()
                            end
                    ...
                    ItemBtn.MouseEnter:Connect(function()
                        TweenService:Create(ItemLabel, TweenInfo.new(0.1), {TextColor3 = Color3.fromRGB(200, 205, 215), TextTransparency = 0}):Play()
                    end)
        ...
        SearchBox:GetPropertyChangedSignal("Text"):Connect(BuildModalItems)

        local DoneConn
        DoneConn = DoneButton.MouseButton1Click:Connect(function()
            DoneConn:Disconnect()
            local HasChanges = false
            if Info.Multi then
                for Val, Act in pairs(TempValues) do
    ...
    function Funcs:AddDropdown(Idx, Info)
        ...
        local ToggleDropdown = function()
            ...
            Library:OpenModalSelector(Dropdown, Info)
        end
        ...
        elseif typeof(Info.Default) == "table" then
            for Key, Value in next, Info.Default do
                local actualVal = typeof(Key) == "string" and Key or Value
                local Index = table.find(Dropdown.Values, actualVal)
                if Index then
                    table.insert(Defaults, Index)
                end
            end
...
function Library:CreateWindow(WindowInfo)
    ...
    do
        ...
        local cleanTitle = WindowInfo.Title:gsub("<[^>]+>", "")
    ...
    function Window:AddTab(...)
        ...
        if select("#", ...) == 1 and typeof(...) == "table" then
            ...
            SingleColumn = Info.SingleColumn
        else
            Name = select(1, ...)
            Icon = select(2, ...)
            Description = select(3, ...)
            SingleColumn = select(4, ...)
        end
        ...
        do
            ...
            TabLabel = New("TextLabel", {
                BackgroundTransparency = 1,
                Position = Icon and UDim2.fromOffset(30, 0) or UDim2.fromOffset(0, 0),
                Size = Icon and UDim2.new(1, -30, 1, 0) or UDim2.new(1, 0, 1, 0),
                RichText = true,
                Text = Name,
                TextSize = 16,
                TextTransparency = 0.5,
            ...
            TabLeft = New("ScrollingFrame", {
                ...
                ScrollBarThickness = 0,
                Size = UDim2.new(SingleColumn and 1 or 0.5, -3, 1, 0),
                Parent = TabContainer,
            })
            New("UIListLayout", {
            ...
            TabRight = New("ScrollingFrame", {
                ...
                ScrollBarImageTransparency = 1,
                ScrollBarThickness = 0,
                Size = UDim2.new(SingleColumn and 0 or 0.5, -3, 1, 0),
        ...
        function Tab:RefreshSides()
            ...
            for i, Side in ipairs(Tab.Sides) do
                Side.Position = UDim2.new(Side.Position.X.Scale, 0, 0, Offset)
                if Tab.SingleColumn then
                    if i == 1 then
                        Side.Size = UDim2.new(1, -3, 1, -Offset)
                    else
    ...
    if Library.IsMobile then
        local ToggleButton = Library:AddDraggableButton("davestudio", function()
            Library:Toggle()
        end, true, true)
    ...
    checkResponsiveLayout = function()
        local isSmall = MainFrame.AbsoluteSize.X < 580
        for _, Tab in pairs(Library.Tabs) do
            Tab.SingleColumn = isSmall

            for _, Groupbox in pairs(Tab.Groupboxes) do
                if Groupbox.Side == 2 then
                    Groupbox.BoxHolder.Parent = isSmall and Tab.Sides[1] or Tab.Sides[2]
                end
            end
            for _, Tabbox in pairs(Tab.Tabboxes) do
                if Tabbox.Side == 2 then
                    Tabbox.BoxHolder.Parent = isSmall and Tab.Sides[1] or Tab.Sides[2]
                end
            end

            local TabLeft = Tab.Sides[1]
            local TabRight = Tab.Sides[2]
            if TabLeft and TabRight then
                TabLeft.Size = UDim2.new(isSmall and 1 or 0.5, isSmall and -6 or -3, 1, 0)
                TabRight.Size = UDim2.new(isSmall and 0 or 0.5, isSmall and 0 or -3, 1, 0)
                TabRight.Visible = not isSmall
            end
    ...
    Library:GiveSignal(MainFrame:GetPropertyChangedSignal("AbsoluteSize"):Connect(checkResponsiveLayout))
    task.spawn(checkResponsiveLayout)
